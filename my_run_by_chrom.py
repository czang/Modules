...
#AnalysisDir = "/home/wpeng/Research/Genome_mapping_of_Chromosome_modification/weiqunmodule";
AnalysisDir = "/home/zang/Modules"
make_graph_file = os.path.join(AnalysisDir, "my_make-graph-file.py");
    ...
    for chrom in chroms:
        ...
        try:
            if os.system('%s -f %s -c %s -w %s -i %s -o %s'%
                         (make_graph_file, bed_file, chrom, window, fragment_size, graph_file)): raise
...
def main(argv):
	...
	parser.add_option("-s", "--species", action="store", type="string",
                      dest="species", help="mm8,hg18", metavar="<str>")
	parser.add_option("-b", "--bed_file", action="store", type="string",
                      dest="bedfile", help="bed file to make graph file of", metavar="<file>")
	...
	parser.add_option("-i", "--fragment_size", action="store", type="string",
                      dest="fragment_size", help="size of fragments after CHIP experiment", metavar="<int>")
	parser.add_option("-o", "--outfile", action="store", type="string",
			dest="outfile", help="output bed summary file name", metavar="<file>")
