...
genomefasta_dir = '/cluster2/czang/genomes/humanhg19/rawgenome/'
...
def logSumLikelihoodRatio(sequence, logpwmlist, logBG0list, threshold):
	size = len(logpwmlist)
	length = len(sequence)
	if length >= size: 
...
def calculate_mm0_bg(bedfile, chroms):
	fafile = bedfile + '.fa'
	if not Utility.fileExists(fafile): 
...
def scan_fasta(fastafile, pwmlist, BG0list, threshold, outfile):
	...
	for line in f:
		if  re.match('>', line):
			...
			line = line.strip()
			score = logSumLikelihoodRatio(line, logpwmlist, logBG0list, threshold)
			o.write(str(score) + '\n')
	f.close()
	o.close()
...
def main(argv):
	...
	parser.add_option("-t", "--threshold", action="store", type="float", dest="threshold", help="score cutoff threshold for every scanning window", metavar="<float>", default = 100)
	...
	scan_fasta(opt.bedfile + '.fa', m, BGlist, opt.threshold, opt.outfile)
