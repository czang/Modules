		...
		if len(islands_1) != 0 and len(islands_2) != 0:
					...
					for i in range(index_start, min(index_end, len(islands_2_end))):
						...
						if OverlapCounts_1 >= float(islands_1[island_index].value) * OverlapThreshold :
							...
							if OverlapCounts_2 >= float (islands_2[i].value) * OverlapThreshold:
								...
								if Normalized_Counts_1 + Normalized_Counts_2 != 0:
						...
						if OverlapConfirmation > 0:  # Real overlapping determination
							f.write(chrom + ' ' + str(islands_1[island_index].start) + ' ' + str(islands_1[island_index].end) + ' ' + str(islands_1[island_index].value)+'\n')
							g.write(chrom + ' ' + str(islands_2[i].start) + ' ' + str(islands_2[i].end) + ' ' + str(islands_2[i].value) + '\n')
							m += 1 # Number of real overlapped pairs. 
							z += 1
							overlap_islands_2_start.append(islands_2[i].start)
					if z == 0:
						h.write(chrom + ' ' + str(islands_1[island_index].start) + ' ' + str(islands_1[island_index].end) + ' ' + str(islands_1[island_index].value)+'\n')
						nh+=1
				else:
					h.write(chrom + ' ' + str(islands_1[island_index].start) + ' ' + str(islands_1[island_index].end) + ' ' + str(islands_1[island_index].value)+'\n')
					nh+=1 
			...
			for i in range (0, len(islands_2_start)): 
				if not (islands_2_start[i] in overlap_islands_2_start):
					k.write(chrom + ' ' + str(islands_2[i].start) + ' ' + str(islands_2[i].end) + ' ' + str(islands_2[i].value) + '\n')
					nk+=1
...
def main(argv):
	...
	separate_by_chrom.separateByChrom(chroms, opt.Histogram1, '.graph1')
	separate_by_chrom.separateByChrom(chroms, opt.Histogram2, '.graph2')
	...
	for chrom in chroms: 
		...
		Histogram1_bed = BED.BED(opt.species, chrom+'.graph1', "BED_GRAPH")
		Histogram2_bed = BED.BED(opt.species, chrom+'.graph2', "BED_GRAPH")
	...
	separate_by_chrom.cleanup(chroms, '.graph1')
	separate_by_chrom.cleanup(chroms, '.graph2')
