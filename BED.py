...
chroms = {"hg18":hg_chroms, "hg19":hg_chroms, "hg38":hg_chroms, "dm2":dm_chroms, "dm3":dm3_chroms, 
          "sacCer1":sacCer_chroms, "mm8":mm_chroms, "mm9":mm_chroms, "mm10":mm_chroms}
...
class BED:
    ...
    def __init__(self, species="hg18", file=None, bed_type="BED3", val_threshold=0):
        ...
        if(file):
            ...
            elif re.match(bed_type, "BED_NAME"):
                """ If want BED_NAME type """
                infile = open(file);
                for line in infile:
                    """ check to make sure not a header line """
                    if not re.match("track", line):
