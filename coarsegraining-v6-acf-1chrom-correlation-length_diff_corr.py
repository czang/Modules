...
def start_list_correlation_r_rev(List, win, r, chrom_length):
	...
	if n - d > 0:
		...
		for i in range(0, n - d):
			sum += a[i] * a[i + d]
		return float(sum)/float(n - d)
	else:
		return 0.0


def correlation2(list1, list2, chrom_length, dx, r):
	s = list1[0]%dx
	keylist1 = []
	keylist2 = []
	for item in list1:
		keylist1.append(item - s)
	for item in list2:
		keylist2.append(item - s)
	#keylist1 = dic1.keys()
	keylist1.sort()
	#keylist2 = dic2.keys()
	keylist2.sort()
	...
	while x < (chrom_length-r):
		...
		x += dx
	return float(total)/(chrom_length - r) *dx
...
def start_list_correlation_function(List, win, chrom_length, name):
	...
	for i in range(0, min(4, chrom_length/win)):
		r = i * win
		c = start_list_correlation_r(List, win, r, chrom_length)
		c2 = start_list_correlation_r_rev(List, win, r, chrom_length)
		c3 = correlation2(List, List, chrom_length, win, r)
		xlist.append(i)
		ylist.append(c)
		file.write(str(i)+'\t'+str(c)+'\t'+str(c2)+'\t'+str(c3)+'\n')
	file.close()
	return (xlist, ylist)
...
def main(argv):
	...
	parser.add_option("-c", "--chrom", action="store", type="string", dest="chrom", help="chromosome", metavar="<str>")
	...
	if opt.species in  GenomeData.species_chroms.keys():
		...
		genome_length = GenomeData.species_chrom_lengths[opt.species][opt.chrom];
		...
		for chrom in chroms: 
			if chrom in bed_val.keys() and len(bed_val[chrom]) > 0:
				...
				islands = traceback(island_list, opt.window_size, opt.step, 0, genome_length, opt.chrom)
