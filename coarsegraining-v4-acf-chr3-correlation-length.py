...
def start_list_correlation_function(List, win, chrom_length):
	xlist = []
	ylist = []
	for i in range(0, min(10, chrom_length/win)):
		r = i * win
		xlist.append(r)
		ylist.append(start_list_correlation_r(List, win, r, chrom_length))
	return (xlist, ylist)
...
def correlation_length_fit(xlist, ylist):
	...
	for i in range(0, len(ylist)):
		loglist.append(log(ylist[i]))
	(a,b,r,stderr) = scipy.stats.linregress(xlist,loglist)
	return -1.0/a
...
def main(argv):
	...
	if opt.species in  GenomeData.species_chroms.keys():
		...
		genome_length = GenomeData.species_chrom_lengths[opt.species]["chr3"];
