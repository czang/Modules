...
def get_gene_float_dic(infile, colum):
	'''returns a dictionary with geneIDs as keys, data as values'''
	file = open(infile, 'r')
	dic = {}
	for line in file:
		if not re.match("#", line):
...
def normalize_percentile_dic(input_file, number, gc, denominator, output_file):
	"""
	number is the 1-based column number for the data to be corrected
	Return a file with same format as input, replacing the column with corrected numbers
...
def main(argv):
	...
	parser.add_option("-a", "--tagcountcolumn", action="store", type="int", dest="colum", metavar="<int>", help="colum number for tag counts to be normalized, start from 1", default=2)
	parser.add_option("-b", "--readpercentile", action="store", type="string", dest="readtable", metavar="<file>", help="GC content density table for read file, must be 100 bins")
	parser.add_option("-c", "--GCcolumn", action="store", type="int", dest="columGC", metavar="<int>", help="colum number GC content density, start from 1",default=2)
	...
	den = get_float_list(opt.readtable, opt.columGC-1)
	gc = get_gene_float_dic(opt.gcdata, 1)
