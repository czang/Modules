...
def normalize_length(input_file, number, total, output_file):
	"""
	...
	for line in infile:
		if not re.match("#", line):
			...
			sline = line.split()
			if len(sline) >= number:
				sline[number-1] = str(atof(sline[0]) * atof(sline[number-1]) / total)
				outline = '\t'.join(sline) +'\n'
...
def total_length(file, column, x = 0):
	total = 0.0
	infile = open(file,'r')
	for line in infile:
		if not re.match("#", line):
			...
			sline = line.split()
			if len(sline) >= column:
				total += atof(sline[0]) * atof(sline[column-1])
	infile.close()
	return total
...
def main(argv):
	...
	if opt.total < 0.01:
		total = total_length(opt.input_file, opt.colum)
	else:
		total = opt.total
	
	normalize_length(opt.input_file, opt.colum, total, opt.output_file)
