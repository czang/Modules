...
def main(argv):
	...
	if Utility.fileExists(opt.boundfile):
		SeparateByChrom.separateByChrom(chroms, opt.boundfile, '.bed2');
	else:
		print opt.bedfile, " not found";
		sys.exit(1)
	...
	for chrom in chroms:
		if chrom in coords.keys():
			genes = coords[chrom];
			if Utility.fileExists(chrom + ".bed2"):
				(gene_name_list, site_list, region_start_list, region_end_list) = associate_island_with_genes.get_gene_bounded_region_lists(coords[chrom], chrom, chrom + ".bed2", opt.promoter_extension)
			else:
				print "error"
			...
			tag_count_list = associate_tags_with_regions.rp(tag_position_list, site_list, region_start_list, region_end_list)
