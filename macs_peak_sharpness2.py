...
Peak sharpness is defined as (average height of 20% region around summit) / (average height of of the peak region) in the wiggle.
...
def calculate_sharpness(start, end, summit, wig_location_list, wig_count_list):
	'''20% around summit / average'''
	start_index = bisect.bisect_right(wig_location_list, start) - 1
	end_index = bisect.bisect_left(wig_location_list, end) 
	assert start_index < end_index
	average = 0.0
	for i in range(start_index, end_index):
		average += wig_count_list[i]
	average = average / (end_index - start_index)
	num = (end - start + 1) / 100
	if num < 1: 
		num = 1
	summit_index = bisect.bisect_right(wig_location_list, summit) - 1
	summit_average = 0.0
	k = 0
	for i in range(summit_index - num, summit_index + num + 1):
		summit_average += wig_count_list[i]
		k += 1
	summit_average = summit_average / k
	return summit_average / average
