...
def main(argv):
	...
	parser.add_option("-n", "--number", action="store", type="int", dest="groupcount", help="total group count", metavar="<int>")
	...
	for i in range(0,opt.groupcount):
		j = str(i+1)
		print j
		separate_by_chrom.separateByChrom(chroms, opt.islandfile1+j, '.island1')
		...
		print "total number of island in "+opt.islandfile1+j+":     ", total_islands_1;
		print "total number of island in "+opt.overlapin1+j+":     ", total_overlap_number_1;
		
		separate_by_chrom.combineAllGraphFiles(chroms, '.1in2', opt.overlapin1+j);
		separate_by_chrom.combineAllGraphFiles(chroms, '.1notin2', opt.nonoverlapin1+j);
