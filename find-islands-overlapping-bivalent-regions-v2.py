...
def main(argv):
	...
	separate_by_chrom.separateByChrom(chroms, opt.Histogram1, '.bed1')
	separate_by_chrom.separateByChrom(chroms, opt.Histogram2, '.bed2')
	
	f = open('bivalent_domains.bed', 'w')
	for chrom in chroms: 
		Histogram1_bed = BED.BED(opt.species, chrom + '.bed1', "BED_GRAPH")
		Histogram2_bed = BED.BED(opt.species, chrom + '.bed2', "BED_GRAPH")
