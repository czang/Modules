...
def main(argv):
	...
	parser.add_option("-k", "--regions_file", action="store", type="string", dest="known_genes", metavar="<file>", help="file with regions in UCSC format")
	...
	for chrom in chroms:
		if chrom in coords.keys():
			...
			current_genes = associate_island_with_genes.place_domains_on_regions(bed_graph_list, genes, opt.start_extension, opt.end_extension, 0.001);
