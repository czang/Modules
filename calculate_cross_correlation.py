...
def all_cross_correlations(extension1, extension2, chroms, dx, file_name, chrom_lengths):
	f = open(file_name, 'w')
	result = {}
	for chrom in chroms:
		dic1 = make_summary_graph_dic(chrom, chrom+extension1)
		...
		chrom_length = chrom_lengths[chrom]
		if len(dic1.keys()) > 0 and len(dic2.keys()) > 0:
			correlation = correlation2(dic1, dic2, chrom_length, dx, 0)
			f.write(chrom+'\t'+str(correlation)+'\n')
		else:
			f.write(chrom+'\t0\n')
	f.close()
...
def main(argv):
	...
	all_cross_correlations('.bed1', '.bed2', chroms, opt.window_size, opt.out_file, chrom_lengths)
