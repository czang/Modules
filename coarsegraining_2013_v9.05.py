#!/usr/bin/env python
# Copyright (c) 2013 The George Washington University
...
def main(argv):
	'''
	Coarse graining test, input result has to be SICER result island file. 
		...
		for chrom in chroms: 
			if chrom in bed_val.keys() and len(bed_val[chrom]) > 0:
				...
				for index in xrange(len(bed_val[chrom])):
					start = bed_val[chrom][index].start
					end = bed_val[chrom][index].end
					x = start
					while x < end:
						eligible_start_list.append(x)
						x += opt.window_size
