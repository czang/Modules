...
def main(argv):
	...
	parser.add_option("-t", "--score", action="store", type="string", dest="threshold", metavar="<string>", help="threshold")
	...
	for line in inputfile:
		if not re.match("#", line):
			...
			if len(sline) > opt.column:
				...
				if flag == opt.threshold:
					total += 1;
					outfile.write('\t'.join(sline)+'\n');
	inputfile.close()
	outfile.close()
	
	print "Total number of genes with ", opt.threshold, "is",  total;
