...
def get_island_regions(bed_vals, Histogram_bed, coords, chrom, TSSthreshold, window_size, p, t, r):
	Result = {}
	Result['total'] = 0
	Result['TSS'] = 0
	Result['GeneBody'] = 0
	...
	if chrom in bed_vals.keys() and chrom in coords.keys():
		...
		for island_index in range(0, len(islands)):
			...
			if abs(average - TSS_List[min(TSSindex,(len(TSS_List)-1))]) < TSSthreshold or abs(average - TSS_List[max(0,(TSSindex-1))]) < TSSthreshold:
				p.write(chrom + '\t' + str(IslandPosition['start']) +'\t' + str(IslandPosition['end']) + '\t' + str(islands[island_index].value) + '\n')
				TSS_Number += 1; 
				...
				t.write(chrom + '\t' + str(IslandPosition['start']) +'\t' + str(IslandPosition['end']) + '\t' + str(islands[island_index].value) + '\n')
				GeneBody_Number += 1;
				...
				r.write(chrom + '\t' + str(IslandPosition['start']) +'\t' + str(IslandPosition['end']) + '\t' + str(islands[island_index].value) + '\n')
				Intergenic_Number += 1 
...
def main(argv):
	...
	parser.add_option("-p", "--outfile1", action="store", type="string", dest="outfile1", metavar="<file>", help="file with promoter islands")
	parser.add_option("-i", "--outfile2", action="store", type="string", dest="outfile2", metavar="<file>", help="file with gene body islands")
	parser.add_option("-r", "--outfile3", action="store", type="string", dest="outfile3", metavar="<file>", help="file with intergenic islands")
	...
	bed_vals = BED.BED(opt.species, opt.bedfile1, "BED_GRAPH")
	if opt.windowsize > 0:
	...
	p = open(opt.outfile1, 'w')
	t = open(opt.outfile2, 'w')
	r = open(opt.outfile3, 'w')
	for chrom in chroms: 
		Result = get_island_regions(bed_vals, Histogram_bed, coords, chrom, opt.TSSthreshold, opt.windowsize, p, t, r)
