...
def result_comparison(Result_1, Result_2, psuedocount, normalization):
	...
	for name in Result_1.keys():
		if Result_2.has_key(name):
			...
			result[name] = float(Result_1[name]) / float(Result_2[name])
	return result
...
def main(argv):
	...
	parser.add_option("-r", "--'Promoter' or 'GeneBody' or 'Gene'", action="store", type="string", dest="region", metavar="<str>", help="region to count tags in")
	...
	bed_vals_3 = BED.BED(opt.species, opt.bedfile3, "BED_GRAPH", 0)
	bed_vals_4 = BED.BED(opt.species, opt.bedfile4, "BED_GRAPH", 0)
	coords = UCSC.KnownGenes(opt.known_genes)
	
	total_tag_1 = 0
	total_tag_2 = 0
	for chrom in chroms: 
		total_tag_1 += get_total_tag_count_on_islands(bed_vals_3[chrom])
		total_tag_2 += get_total_tag_count_on_islands(bed_vals_4[chrom])
	Normalization = float(total_tag_1) / float(total_tag_2)
	
	Result = {}
	
	separate_by_chrom.separateByChrom(chroms, opt.bedfile3, '.bed1')
	separate_by_chrom.separateByChrom(chroms, opt.bedfile4, '.bed2')
	for chrom in chroms: 
		Histogram_bed_1 = BED.BED(opt.species, chrom+'.bed1', "BED_GRAPH")
		Histogram_bed_2 = BED.BED(opt.species, chrom+'.bed2', "BED_GRAPH")
