...
'''version 5: 3-phase coarse graining, take the phase that has most 1 to next step.
   Trace-back: merging all islands that are separated by gaps less than the current window size.
'''
...
def coarsegraining(List, win_min, step, score, limit):
	...
	win = win_min
	while len(List) > 0 and len(result_list) <= limit:
		print len(List)
...
def main(argv):
	...
	parser.add_option("-n", "--steplimit", action="store", type="int",  dest="limit", help="maximum step of block transformation (>1)", metavar="<int>")
	...
	if opt.species in  GenomeData.species_chroms.keys():
		...
		#window_pvalue = opt.window_pvalue;
		bin_size = 0.001;
		...
		for chrom in chroms: 
			if chrom in bed_val.keys() and len(bed_val[chrom]) > 0:
				...
				(result_list, island_list) = coarsegraining(eligible_start_list, opt.window_size, opt.step, opt.score, opt.limit)
