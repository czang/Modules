...
def main(argv):
	...
	parser.add_option("-t", "--upextension", action="store", type="int", dest="start", metavar="<int>", help="upstream extension")
	parser.add_option("-e", "--downextension", action="store", type="int", dest="end", help="downstream extension", metavar="<int>")
	...
	for chrom in chroms:
		if chrom in coords.keys():
			genes = coords[chrom];
			(gene_name_list, region_start_list, region_end_list) = associate_island_with_genes.get_promoter_feature_region_lists(coords[chrom], opt.start, opt.end)
