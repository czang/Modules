...
def main(argv):
	...
	parser.add_option("-t", "--mappable_faction_of_genome_size ", action="store", type="float", dest="fraction", help="mapable fraction of genome size", metavar="<float>", default=150)
	...
	print "Library size  ", chip_library_size
			...
			if len(islands[chrom]) != 0:
				...
				for index in xrange(len(island_list)):
					...
					if (island_control_readcount[chrom])[index] > 0:
						...
						average = min(1.0, average)* scaling_factor;
						fc = float(observation)/float(average);
					...
					#item_dic['fc'] = fc
					outline = item.chrom + "\t" + str(item.start) + "\t" + str(item.end) + "\t" + str(observation) + "\t" + str(control_tag) + "\t" + str(pvalue) + "\t" + str(fc) + "\n";
					out.write(outline);
	...
	#	outline = item['chrom'] + "\t" + str(item['start']) + "\t" + str(item['end']) + "\t" + str(item['chip']) + "\t" + str(item['control']) + "\t" + str(item['pvalue']) + "\t" + str(item['fc']) + "\n";	
	#	out.write(outline);		
	out.close();
