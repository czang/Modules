...
def main(argv):
	...
	bed_vals = BED.BED(opt.species, opt.bedfile, "BED_NAME", 0); 
	for chrom in chroms:
	...
	for chrom in chroms:
		if chrom in coords.keys():
			...
			current_genes = associate_island_with_genes.place_domain_names_on_genes(bed_graph_list, genes, opt.region_type, opt.promoter_extension);
			for gene in current_genes.keys():
				occupied_genes[gene] = current_genes[gene]
	
	f = open(opt.out_file, 'w')
	for g in occupied_genes.keys():
		f.write(g + '\t' + '\t'.join(occupied_genes[g]) + '\n')
	f.close()
