...
def main(argv):
	parser = OptionParser()
	parser.add_option("-a", "--islandfile1", action="store", type="string", dest="islandfile1", metavar="<file>", help="file 1 with islands info to be compared, will use a sub region from center")
	parser.add_option("-b", "--islandfile2", action="store", type="string", dest="islandfile2", metavar="<file>", help="MACS 1.4 summits bed file")
	...
	parser.add_option("-l", "--regionsize", action="store", type="int", dest="region_size", help="region size around the center of each island", metavar="<int>")
		...
		if fileExists(chrom+'.island1') and len(bed_vals_2[chrom])>0:
			...
			for line in islands1:
				if not re.match("#", line):
					...
					center = (start + end) / 2
					left = center - regionsize / 2
					right = center + regionsize / 2
					if (region_overlap(left, right, island2_start_list, island2_end_list) == 1):
						f.write('\t'.join(sline) + '\n')
