...
def main(argv):
	...
	for chrom in chroms:
		if chrom in coords.keys():
			...
			current_genes = associate_island_with_genes.place_domains_on_gene_ends(bed_graph_list, genes, opt.promoter_extension, opt.promoter_extension);
			for gene in current_genes.keys():
				if current_genes[gene] > threshold:
					occupied_genes[gene] = current_genes[gene]
				else:
					occupied_genes[gene] = 0.0
