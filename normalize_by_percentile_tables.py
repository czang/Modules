...
def normalize_percentile(input_file, number, gc, numerator, denominator, output_file):
	"""
	...
	for line in infile:
		if not re.match("#", line):
			...
			if len(sline) >= number:
				...
				index = int(gc[i]*100)
				norm = raw / denominator[index] * numerator[index]
...
def main(argv):
	...
	normalize_percentile(opt.input_file, opt.colum, gc, num, den, opt.output_file)
