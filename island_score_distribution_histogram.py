...
def main(argv):
	...
	if len(argv) < 6 or opt.option not in [1,2]:
        	...
        	sys.exit(1)
	if opt.option == 1: 
		islands_statistics_pr.find_islands_score_histogram({}, opt.input_file, 1, opt.output_file)
	elif opt.option == 2: 
		islands_statistics_pr.find_islands_score_histogram({}, opt.input_file, 1, "pdftemp")
