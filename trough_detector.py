...
def score_list(start, count_list, w, step = 10):
	N = w/step
	l = len(count_list)
	avg = []
	for i in range(0,l-N):
		count = sum(count_list[i:i+N])
		avg.append(count)
	scores = []
	for i in range(0,l - N * 3):
		score = (avg[i] + avg[i + N * 2])/2 - avg[i + N]
		scores.append(max(score,0))
	return scores, int(start + w*1.5)
...
def main(argv):
	parser = OptionParser()
	parser.add_option("-s", "--species", action="store", type="string", dest="species", help="species, e.g., hg19, mm9", metavar="<str>")
	parser.add_option("-b", "--bedfile", action="store", type="string", dest="bedfile", metavar="<file>", help="island file")
	parser.add_option("-g", "--wiggle", action="store", type="string", dest="wig", metavar="<file>", help="wig path and file except chroms, must separate chromosomes")
	parser.add_option("-e", "--window", action="store", type="int", dest="windowsize", metavar="<int>", help="window size for trough detection", default=200)
	parser.add_option("-w", "--outwig", action="store", type="string", dest="out_wig", metavar="<file>", help="output wiggle file name")
	parser.add_option("-o", "--outfile", action="store", type="string", dest="out_file", metavar="<file>", help="output trough center file name")
		...
		if Utility.fileExists(opt.wig+chrom+'.wig'):
			...
			if len(islandlist) > 0:
				...
				for item in islandlist:
					...
					if len(rawList) >= (opt.windowsize * 3 / 10): 
						(scores, new_start) = score_list(start, rawList, opt.windowsize)
						for item in scores: 
							ylist.append(item)
				...
				f.close()
				print 'writing troughs for', chrom
