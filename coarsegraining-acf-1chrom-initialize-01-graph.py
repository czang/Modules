...
def start_list_correlation_function(List, win, chrom_length, name):
	...
	for i in range(0, min(4, int(chrom_length/win))):
		r = i * win
...
def main(argv):
	...
	if opt.species in  GenomeData.species_chroms.keys():
		...
		print "Generate initialized bedgraph"; 
		#read in the summary graph file
		f = open(opt.summarygraph,'r')
		...
		for line in f:
			if not re.match("#", line):
				...
				if len(sline)>3:
					count = atoi(sline[3])
					if count >= min_tags_in_window:
						o.write('\t'.join([sline[0], sline[1],sline[2],'1'])+'\n')
					else:
						o.write('\t'.join([sline[0], sline[1],sline[2],'0'])+'\n')
		f.close()
		o.close()
...
'''		bed_val = BED.BED(opt.species, opt.summarygraph, "BED_GRAPH");
		...
		for chrom in chroms: 
			if chrom in bed_val.keys() and len(bed_val[chrom]) > 0:
				...
				islands = traceback(island_list, opt.window_size, opt.step, 0, genome_length, opt.out_file)
