...
def main(argv):
	...
	parser.add_option("-c", "--controlflanking", action="store", type="string", dest="cf_file", metavar="<file>", help="control flanking data file")
	parser.add_option("-d", "--treatedflanking", action="store", type="string", dest="tf_file", metavar="<file>", help="treated data file")
	...
	control_average = mean(control_center) + mean(control_flanking)
	treated_average = mean(treated_center) + mean(treated_flanking)

	out = open(opt.out_file, 'w')
	for i in range(0,len(tf_names)):
		score = sqrt(treated_flanking[i]/treated_average)-sqrt(control_flanking[i]/control_average)-sqrt(treated_center[i]/treated_average)+sqrt(control_center[i]/control_average)
