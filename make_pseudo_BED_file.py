	...
	for line in infile:
		if not re.match("#", line):
			...
			if len(sline) >= max(name, chrom, strand, start, end):
				if strand > 0:
					outfile.write(chromname(sline[chrom - 1]) + '\t' + sline[start - 1] + '\t' + sline[end - 1] + '\t' + sline[name - 1] + '\t' + '0' + '\t' + strandsign(sline[strand - 1]) + '\n')
				else:
					outfile.write(chromname(sline[chrom - 1]) + '\t' + sline[start - 1] + '\t' + sline[end - 1] + '\t' + sline[name - 1] + '\t' + '0' + '\t+\n')
	infile.close()
	outfile.close()
