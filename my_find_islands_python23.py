...
##  parameters
window_count_threshold = 2;
reference_islands_minimum_tags = 8;
reference_total_tag_counts = 1400000.0;

window_buffer = 10;

species = ['mm8','hg18','background'];
    ...
    while island_index < (len(islands) - 1):
        ...
        while extend:
            if island_index < (len(islands) - 1):
                ...
                got_last = 1;
        island_index += 1;
        whole_island = MYBED.BED_GRAPH(i.chrom, start, end, score);
...
def findIslands(bed_vals, chrom, outfilename, window, gap, islands_minimum_tags):
    ...
    if chrom in bed_vals.keys():
        tags = bed_vals[chrom];
	# sort with key is only available in python 2.4
        #tags.sort(key=operator.attrgetter('start')); 
	tags.sort(lambda x, y: cmp(x.start, y.start));
        tag_index = 0;
        got_last = 0;
            ...
            while extend:
                if tag_index < (len(tags) - 1):
                    ...
                    if next_distance < window_buffer:
                        ...
                        island = MYBED.BED_GRAPH(chrom, island_start, island_end, island_value);
...
def find_region(bed_vals, chrom, gap, window_size, window_size_buffer=10):
	"""
		bed_graph_file is the bed_graph type summary file
    		...
    		return a list of MYBED.BED_GRAPH, each of which has (i.chrom, start, end, score)
    	"""  
...
def main(argv):
	...
	parser.add_option("-t", "--min_tag_count_in_island", action="store", type="int",
			dest="islands_minimum_tags", help="minimum number of tags required in each island; if <0, self-adjust; if>=0 use it", metavar="<int>")
	(opt, args) = parser.parse_args(argv)
	if len(argv) < 14:
		parser.print_help()
		sys.exit(1)

        if opt.species in species:
                total_tag_counts = get_total_tag_counts.get_total_tag_counts_bed_graph(opt.bedfile);
                
		if (opt.islands_minimum_tags<0):
			islands_minimum_tags = int(round((total_tag_counts/reference_total_tag_counts)*reference_islands_minimum_tags));
		else: 	
			islands_minimum_tags = opt.islands_minimum_tags;


            	"""  Get all bed vals with a score >= window_count_threshold"""
            	bed_vals = MYBED.BED(opt.species, opt.bedfile, "BED_GRAPH", opt.tag_count_threshold_in_window);
		
	    	
		#islands_minimum_tags = 0;
	    	print opt.bedfile, "          total tag counts is "+ str(total_tag_counts), "       Island threshold is "+str(islands_minimum_tags);
		...
		outputfile = open(opt.outfile, 'w');
			...
			for i in islands:
                ...
                if islands_minimum_tags == 0:
			tempname = ((opt.bedfile).split('/'))[-1];
            		histogram_file = (tempname.split('.'))[0] + '.' + (tempname.split('.'))[1] + "_island_histogram"+"_g" + str(opt.gap)+"_m" + \
				str(opt.tag_count_threshold_in_window) +".dat"; 
			print "Now calculating the hisogram, output to " + histogram_file;  
            		get_islands_histogram.find_region_histogram_2 (opt.species, total_tag_counts, opt.outfile, histogram_file);
        else:
