...
chroms = {"hg18":hg_chroms, "dm2":dm_chroms,
          "sacCer1":sacCer_chroms, "mm8":mm_chroms}
...
class Starts:
    ...
    def __init__(self, species="hg18", file=None, fragmentsize=300):
        """
		...
		if sline[0] in self.bed_vals.keys():
				...
				position = atoi(sline[1]) + fragmentsize/2;
                		self.bed_vals[sline[0]].append(position);
                	elif minus.match(sline[5]):
				position = max(0, atoi(sline[2])- fragmentsize/2);
                		self.bed_vals[sline[0]].append(position);
