...
def main(argv):
	...
	bed_vals = BED.BED(opt.species, opt.bedfile, "BED6", 0);
	for chrom in chroms:
	...
	for chrom in chroms:
		if chrom in coords.keys():
			genes = coords[chrom];
			total += len(genes);
			bed_list = bed_vals[chrom];
			current_genes = associate_island_with_genes.place_domains_maxscore_w_strand_on_genes(bed_list, genes, opt.region_type, opt.promoter_extension);
