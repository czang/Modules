...
def getScoresNearPoints(coords, bed_vals, shift):
...
def main(argv):
    parser = OptionParser()
    parser.add_option("-k", "--known_genes_file", action="store", type="string", dest="known_file", help="file with known genes", metavar="<file>")
    parser.add_option("-b", "--bedfile", action="store", type="string", dest="bedfile", help="file with tags in bed format", metavar="<file>")
    parser.add_option("-f", "--fragsize", action="store", type="int", dest="fragsize", help="average ChIP fragment size", metavar="<int>", default = 150)
    parser.add_option("-o", "--outfile", action="store", type="string", dest="outfile", help="outfile name", metavar="<file>")
    parser.add_option("-n", "--normalize", action="store_true", dest="norm", help="normalize by tag number")
	...
	if chrom in coords.keys():
		...
		if fileExists(chrom+'.bed'):
			...
			scores[chrom] = getScoresNearPoints(coords, bed_vals, opt.fragsize / 2);
