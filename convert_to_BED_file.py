...
"""
This module is used to make a BED format file that meet the format requirement 
http://genome.ucsc.edu/goldenPath/help/customTrack.html#BED
The input file can be any format gene file given name, chromosome, strand, start position and end position in differnt columns.
...
def get_formated_genes(input_file, name, chrom, strand, start, end, score, output_file):
	"""
		...
		if not re.match("#", line):
			...
			if len(sline) >= max(name, chrom, strand, start, end):
				if score > 0:
					...
					s = '0'
				if strand > 0:
					outfile.write(chromname(sline[chrom - 1]) + '\t' + sline[start - 1] + '\t' + sline[end - 1] + '\t' + sline[name - 1] + '\t' + s + '\t' + strandsign(sline[strand - 1]) + '\n')
				else:
					outfile.write(chromname(sline[chrom - 1]) + '\t' + sline[start - 1] + '\t' + sline[end - 1] + '\t' + sline[name - 1] + '\t' + s + '\t+\n')
	infile.close()
	outfile.close()
...
def main(argv):
	...
	parser.add_option("-f", "--score", action="store", type="int", dest="score", metavar="<int>", help="colum number for score, all 0 if 0")
	...
	get_formated_genes(opt.input_file, opt.name, opt.chrom, opt.strand, opt.start, opt.end, opt.score, opt.output_file)
