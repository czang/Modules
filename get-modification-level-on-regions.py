...
def main(argv):
	...
	parser.add_option("-p", "--tagcountpvalue", action="store", type="float", dest="pvalue", help="P value, only windows those have a probability of having such number of tags is less than p are counted", metavar="<float>")
	...
	for chrom in chroms:
		if chrom in coords.keys():
			...
			current_genes = associate_island_with_genes.place_domains_on_regions(bed_graph_list, genes, opt.start_extension, opt.end_extension, threshold);
			for gene in current_genes.keys():
				if current_genes[gene] >.01:
					...
					occupied_genes[gene] = pseudocount/float(total_tag_counts)
