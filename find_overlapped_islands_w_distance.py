...
def region_overlap(start, end, start_list, end_list, distance):
	"""
	Assuming non overlapping islands.
	"""
	assert (start <= end);
	start_position = bisect.bisect_right(end_list, start - distance);
	end_position = bisect.bisect_left(start_list, end + distance); 
...
def main(argv):
	parser = OptionParser()
	parser.add_option("-s", "--species", action="store", type="string", dest="species", help="species in genome version, eg, hg18", metavar="<str>")
	...
	parser.add_option("-d", "--distance", action="store", type="int", dest="distance", help="maximum distance between islands, in bp", metavar="<int>")
	...
	for chrom in chroms: 
		...
		if len(bed_vals_1[chrom]) > 0  and len(bed_vals_2[chrom])>0:
			...
			for islandlist1_item in islandlist1:
				...
				if (region_overlap(start, end, island2_start_list, island2_end_list, opt.distance) == 1):
