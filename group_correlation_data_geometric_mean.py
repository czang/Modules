...
def main(argv):
	...
	for i in range(0, opt.groupnumber):
		data_dic = get_positive_log_data_dic(opt.inputfile+'_'+str(i+1), 1)
		data_dic_2 = get_positive_log_data_dic(opt.inputfile+'_'+str(i+1), 2)
		Average_List.append(pow(10,average_expression_of_genes(data_dic, data_dic.keys())))
		Average_List_2.append(pow(10,average_expression_of_genes(data_dic_2, data_dic.keys())))
	
	f = open(opt.out_file, 'w')
	for i in range(0, len(Average_List)):
		f.write(str(i+1)+'\t'+str(Average_List[i])+'\t'+str(Average_List_2[i])+'\n')
	f.close()
